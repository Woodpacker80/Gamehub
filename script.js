...
function startRound(){
  ...
  if(roundType !== 'normal'){
    ...
    if(roundType === 'gold'){
      ...
      body  = 'A Gold BONUS tile (+$5,000) and a Silver BONUS tile (+$2,500) are on the wheel. Two <span class="star-blue">★</span> JOKER tiles and one Minus tile are also in play — the Minus tile costs you 50% of your round earnings. Land on a BONUS tile and guess a consonant to collect it! Vowels cost $500 this round!';
      icon  = '👑';
    } else {
...
function updateTurnUI(){
  ...
  // Vowel buying allowed without spinning when no consonants remain
  const canBuyVowel = isP && !G.locked && myScore >= vCost && hasUnrevealedVowels() && !G.suddenDeath &&
    (G.phase==='guess' || (G.phase==='spin' && G.hasGuessed) || (G.phase==='spin' && noConsLeft));
  // Spinning not allowed when no consonants remain
...
function handleBuyVowel(){
  ...
  if(G.phase === 'spin' && !G.hasGuessed && !_noConsLeft) return;
  ...
  setTimeout(()=>{
    ...
    if(hit){
      ...
      if(isVowel){
        ...
        // Banner: wheel closed if no consonants remain
        if(!hasUnrevealedConsonants()){
          setTimeout(()=>setBanner('🔒 Only vowels left! Wheel is closed.','neutral'), 800);
        }
        if(!G.mpMode && who==='computer') setTimeout(()=>computerTurn(),1300);
      }
    } else {
...
function computerTurn(){
  ...
  setTimeout(()=>{
    ...
    // ── 1. 1 position missing — always solve ─────────────────────────────────
    if(missingPositions <= 1){
      setBanner('🤖 Solving…','money');
      setTimeout(()=>endRound('computer'), 1400); return;
    }

    // ── 2. Vowel trap fix — no consonants left ────────────────────────────────
    if(noConsonantsLeft){
      // Do not spin — go straight to buying a vowel or forced solve
      ...
      // Forced solve — below threshold = Illusionist fail
      if(pct < cfg.solve){
        // Illusionist: 2-stap TV-show spanning
        showResult('Computer is solving...', 'The computer is trying to guess the word!', '🤖', false, null);
        setTimeout(()=>{
          document.getElementById('modal-result').classList.remove('open');
          setTimeout(()=>{
            showResult('Computer Failed', 'The computer guessed wrong. It is your turn!', '🤖', false, ()=>switchTurn('computer'));
          }, 250);
        }, 2000); return;
      }
      setBanner('🤖 Solving…','money');
    ...
    // ── 3. Regular solve attempt with threshold + Illusionist ────────────────
    ...
    // ── 5. Spin — only if consonants remain ─────────────────────────────────
    if(noConsonantsLeft){
      // No consonants left but no other option — forced solve as fallback
      setBanner('🤖 Solving…','money');
      setTimeout(()=>endRound('computer'), 1400); return;
    }
    doSpin('computer');
...
async function arcadeHighscoreFlow(score, difficulty){
  ...
  try {
    ...
    if(sub) sub.textContent = 'Congratulations! You made the Top 10!';
...
async function arcadeHsSave(){
  ...
  if(name.length < 2){
    if(err) err.textContent = 'At least 2 characters required';
    return;
  }
  if(!/^[A-Z0-9 _\-\.]+$/.test(name)){
    if(err) err.textContent = 'Letters, numbers and . _ - only';
    return;
  }
...
async function showWinLeaderboard(difficulty, myScore){
  return new Promise(async (resolve)=>{
    try {
      ...
      const rankIcons= ['🥇','🥈','🥉'];
      const rankClass= ['gold','silver','bronze'];
      // myScore !== null means we just submitted — highlight our entry
      ...
      list.innerHTML = entries.map((e,i)=>{
        ...
        return `<div class="lb-entry${isMe?' lb-you':''}${isNew?' lb-new-entry':''}">
          ${rank}
          <span class="lb-name">${e.name}${isNew?' ◀ you':''}</span>
          <span class="lb-score">$${e.score.toLocaleString()}</span>
        </div>`;
      }).join('');
      // Show for 3 seconds then hide and resolve
      setTimeout(()=>{
        overlay.style.opacity='0';
        setTimeout(()=>{
          overlay.classList.remove('visible');
