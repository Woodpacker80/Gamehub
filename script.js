...
function drawWheel(rot){
  ...
  for(let i=0;i<n;i++){
    ...
    if(seg.val==='BONUS_GOLD'){
      ...
      ctx.fillStyle=g;
      ctx.fill();
      // Red flakes edge to edge at 70% density
...
function startRound(){
  ...
  if(roundType !== 'normal'){
    ...
    if(roundType === 'gold'){
      title = '👑 Gold Round!';
      body  = 'A Gold BONUS tile (+$5,000) lands next to Bankrupt and a Silver BONUS tile (+$2,500) is also on the wheel. Two JOKER tiles and one Minus tile are also in play. Vowels cost $500 this round!';
      icon  = '👑';
    } else {
      title = '🥈 Silver Round!';
...
function updateTurnUI(){
  ...
  document.getElementById('btn-solve').disabled = G.phase==='guess' ? false : !(isP&&!G.locked);
...
function handleJoker(who){
  ...
  if(unrevealed.length === 0){
    ...
    setBanner('🃏 Joker — no letters left! Turn continues.','money');
    setTimeout(()=>{
...
function endGame(){
  ...
  if(p>c){
    icon='🏆'; title='You Win!';
    sub=`You beat the computer by $${(p-c).toLocaleString()}!`;
    outcome='win'; sndFanfare();
  } else if(c>p){
    icon='🥈'; title='Computer Wins';
    sub=`Well played! Difference: $${(c-p).toLocaleString()}.`;
    outcome='loss';
  } else {
