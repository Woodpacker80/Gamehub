...
function drawWheel(rot){
  ...
  // We need two passes: first draw all fills+borders, then sparkles on top
  const sparkleSegs=[];
...
function startRound(){
  ...
  G.turn='player'; G.suddenDeath=false; G.bonusVal=0; G.onBonus=false;
  ...
  if(roundType === 'chrome'){
    // Chrome round: inject Chrome bonus tile + Minus tile at random money spots
    const base = [...SEGS];
    ...
    base.splice(positions[0], 0, BONUS_SEGS.chrome[0]); // Chrome bonus
    base.splice(positions[1], 0, BONUS_SEGS.chrome[1]); // Minus
    activeSegs = base;

  } else if(roundType === 'gold'){
    // Gold round — precise wheel construction:
    // 1. Start from base SEGS
    // 2. Place Gold tile immediately after BANKRUPT
    // 3. Inject Chrome tile at a random money position
    // 4. Swap ONE of the two LOSE tiles to MINUS (leave the other)
    // 5. Replace ONE random money tile with second MINUS
    const base = [...SEGS];

    // Step 1: Gold tile next to BANKRUPT
    const bkIdx = base.findIndex(seg => seg.val === 'BANKRUPT');
    base.splice(bkIdx + 1, 0, BONUS_SEGS.gold[0]); // Gold bonus after BANKRUPT

    // Step 2: Chrome tile at a random money position
    ...
    base.splice(chromePos, 0, BONUS_SEGS.chrome[0]); // Chrome bonus

    // Step 3: Swap exactly ONE LOSE tile to MINUS (first one found)
    const loseIdx = base.findIndex(seg => seg.val === 'LOSE');
    if(loseIdx >= 0) base[loseIdx] = { lbl:'MINUS', val:'MINUS', col:'#c0392b' };

    // Step 4: Replace ONE random money tile with second MINUS
    // Enforce minimum 4-segment gap between the two Minus tiles (circular distance)
    ...
    const MIN_GAP    = 4;
    // Shuffle money positions and pick the first one that is far enough from Minus 1
    const shuffled2  = [...moneyIdxs2].sort(()=>Math.random()-0.5);
    let minusPos = shuffled2[0]; // fallback
  ...
  if(roundType === 'gold'){
    pill.textContent = '👑 Gold Round ' + G.round + ' of ' + G.maxRounds;
    ...
    pill.textContent = '⭐ Chrome Round ' + G.round + ' of ' + G.maxRounds;
  ...
  if(roundType !== 'normal'){
    ...
    if(roundType === 'gold'){
      title = '👑 Gold Round!';
      body  = 'Gold Bonus (+$5,000) lands next to Bankrupt. Chrome Bonus (+$2,500) is also on the wheel. One Lose a Turn is now Minus (−50%). Vowels cost $500 this round!';
      icon  = '✨';
    } else {
      title = '⭐ Chrome Round!';
      body  = 'Chrome Bonus tile: land it + correct consonant = earnings + $2,500! Minus tile: lose 50% round earnings!';
      icon  = '⚡';
    }
    setTimeout(()=>{
...
function updateTurnUI(){
  ...
  const _vc=G.vowelCost||VOWEL_COST; document.getElementById('btn-vowel').disabled = !(isP&&(G.phase==='spin'||G.phase==='guess')&&!G.locked&&G.roundScore.player>=_vc&&hasUnrevealedVowels());
...
function isDeadlocked(){
  ...
  const _vc2=G.vowelCost||VOWEL_COST; return !hasUnrevealedConsonants()&&G.roundScore.player<_vc2&&G.roundScore.computer<_vc2&&hasUnrevealedVowels();
}
...
function doSpin(who){
  ...
  const dur=4000+Math.random()*1200; // longer duration for quintic tail
  const t0=performance.now(), startRot=wheelRot;
  let lastSeg=-1;

  function frame(now){
    const elapsed=now-t0, prog=Math.min(elapsed/dur,1);
    const ease=1-Math.pow(1-prog,5); // quintic ease-out — dramatic Hollywood tail
...
function onLanded(seg,who){
  if(seg.val==='BANKRUPT'){
    ...
    setTimeout(()=>showResult(
      who==='player'?'💸 BANKRUPT!':'🤖 Bankrupt!',
      (who==='player'?'You lose':'Computer loses')+' all round earnings!',
      '😱',false,()=>switchTurn()),500);

  } else if(seg.val==='LOSE'){
    sndWrong(); setBanner('😬 LOSE A TURN','neutral');
    setTimeout(()=>showResult(
      who==='player'?'😬 Lose a Turn':'🤖 Lose a Turn',
      (who==='player'?'You lose':'Computer loses')+' this turn!',
      '😞',false,()=>switchTurn()),500);
    ...
    setTimeout(()=>showResult(
      ...
      (who==='player'?'You lose':'Computer loses')+' '+lostStr+' from round earnings!',
      '😱',false,()=>switchTurn()),600);

  } else if(seg.val==='BONUS_CHROME'){
    G.spinVal=800; G.bonusVal=2500; G.onBonus=true;
    sndBonusLand(); flashBonusHub();
    setBanner('🥈 CHROME BONUS — guess a letter!','bonus');
...
function handleBuyVowel(){
  ...
  const vc = G.vowelCost || VOWEL_COST;
  if(G.roundScore.player<vc||!hasUnrevealedVowels()) return;
  G.phase='buy_vowel'; G.spinVal=0; G.locked=false;
  setBanner('🔤 Pick a vowel — costs $' + vc,'money'); updateTurnUI();
}
  ...
  setTimeout(()=>{
    ...
    if(hit){
      ...
      if(!isVowel && G.spinVal){
        ...
        earn = ` = $${le.toLocaleString()}`;
        if(G.onBonus && G.bonusVal){ const icon=G.roundType==='gold'?'👑':'🥈'; earn += ' + $'+G.bonusVal.toLocaleString()+' '+icon+' BONUS!'; }
      }
...
function computerTurn(){
  ...
  setTimeout(()=>{
    ...
    // Buy vowel?
    if(G.roundScore.computer>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&pct>0.15&&Math.random()<cfg.vowel){
